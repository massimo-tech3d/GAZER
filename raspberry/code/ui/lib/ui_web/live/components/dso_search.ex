defmodule UiWeb.Components.DeepSkySearch do
  ...
  def render(assigns) do
    ...
    <fieldset class="flex flex-col basis-auto p-1 mx-1">
        <.form let={f} for={:dso_specs} phx-submit="dso_go" phx-change="dso_filter" phx-target={@myself} class="container flex flex-col items-center">
          <%= label f, :limit_magnitude, class: "text-s" %>
            ...
            <div>
              ...
              <%#= checkbox(f, :messier_only, [class: "rounded-md focus:ring-0 bg-gray-500 accent-emerald-600 checked:bg-neutral-800 my-2", checked: @messier_only=="true"]) %>
              <%= checkbox(f, :messier_only, [class: "rounded-md focus:ring-0 border border-red-700 bg-black checked:text-black checked:bg-black checked:border-red-700 my-2", checked: @messier_only=="true"]) %>
            </div>
            <div class="flex flex-col grow-0">
              <div class="flex mt-3">
                <span class="flex items-center px-3 pointer-events-none text-s rounded-l-md border border-red-700">Object type</span>
                <%= select(f, :dso_filter, @choices, [selected: @dso_type, class: "border text-s rounded-r-md border-red-700 text-gray-500 bg-neutral-800"]) %>
              </div>
              <%= select(f, :object, @objects, [objects: @object, class: "border mt-3 text-xs rounded-md border-red-700 text-gray-500 bg-neutral-800"]) %>
              <%= submit "Go", class: "bg-neutral-800 hover:bg-neutral-700 text-sm font-bold py-2 px-4 rounded my-3"%>
            </div>
        </.form>
      </fieldset>
    </div>
