...
void axes_FXOS(float raw[3], bool flat) {
...
void accel_readings_FXOS(float accel_raw[3], bool flat=false) {
...
void mag_readings_FXOS(float mag_raw[3], bool flat=true) {
    mag_readings_avg_FXOS(mag_raw);

    axes_FXOS(mag_raw, flat); // no need to swap axes (perchè ho messo questa nota ?
}
